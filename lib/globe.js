...
class Globe {
  ...
  findCountry(event) {
    ...
    const country = this.worldMap.features.find(f => d3.geoContains(f, pos));
  ...
  render() {
    ...
    ctx.globalAlpha = 0.7;
    Util.drawObj(this.water, this.colors.water);
    Util.drawLine(this.worldMap, this.colors.land);
    Util.drawLine(this.grat, this.colors.grat);

    if (this.highlight) {
      Util.drawObj(this.highlight, 'red');
    }
  }
}
