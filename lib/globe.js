...
class Globe {
  constructor(dims) {
    ...
    this.currentListItem = this.whoList[3];
  ...
  handleClick() {
    ...
    if (country) {
      // const queries = Util.getCountry(this.currentListItem, country.id);
      $.when(...Util.getCountry(this.currentListItem, country.id)).done((...args) => {
        debugger
      })
  ...
  render() {
    ...
    if (this.renderDims === this.dims) {
      DrawUtil.drawTitle(ctx, this.worldMap.features.find(el => el.fact).fact.title, this.renderDims);
    }
