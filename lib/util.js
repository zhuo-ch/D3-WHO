...
export const fetchWHO = (listItem) => {
  ...
  return $.ajax({
    method: 'GET',
    dataType: 'jsonp',
    url: `http://apps.who.int/gho/athena/api/GHO/${globeSource}?filter=YEAR:2015&format=json`,
  });
}

export const fetchCountry = (listItem, id) => {
  const pieSource = listItem.pieSource;
  const pieFilter = listItem.pieFilter;

  return $.ajax({
    method: 'GET',
    dataType: 'jsonp',
    url: `http://apps.who.int/gho/athena/api/GHO/${pieSource}?filter=COUNTRY:${id};${pieFilter};YEAR:2015&format=json`,
  });
}
...
export const whoList = [
  ...
  },
  {
    ...
    pieSource: ['MEDS2020200C', 'DEVICES00', 'DEVICES01', 'DEVICES02', 'DEVICES03', 'DEVICES04', 'DEVICES005'],
    pieFilter: '',
  },
  {
    title: 'Poverty',
