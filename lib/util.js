...
export const formatMultipleIndicators = data => {
  ...
  data.forEach(f => {
    const indicator = f[0].dimension.find(el => el.display === 'Indicator');
    if (indicator) {
      facts.push({
  ...
  return facts.length > 0 ? facts.sort((a, b) => sortFacts(a, b)) : [{ label: 'null', display: 'No Data', value: 0 }];
}
...
const parseValue = value => {
  ...
  if (val.length === 2 && val[1].length > 2) {
    const parsedVal = val[0] + val[1].slice(0, 2);
