...
var who = exports.who = function who() {
  return $.ajax({
    method: 'GET',
    dataType: 'jsonp',
...
var fetchWHO = exports.fetchWHO = function fetchWHO(listItem) {
  var globeSource = listItem.globeSource;
  var globeFilter = listItem.globeFilter ? 'filter=' + listItem.globeFilter + '&' : '';
...
var getCountry = exports.getCountry = function getCountry(listItem, id) {
  return listItem.pieSource.map(function (source, idx) {
    return createAjax(listItem, id, idx);
  });
};

var createAjax = exports.createAjax = function createAjax(listItem, id, idx) {
  var pieSource = listItem.pieSource[idx];
  var pieFilter = listItem.pieFilter ? ';' + listItem.pieFilter : '';
...
var bindMap = exports.bindMap = function bindMap(map, facts) {
  var _loop = function _loop(i) {
    map.features[i].fact = facts.find(function (fact) {
      return map.features[i].id === fact.country;
    });
  };
...
var formatWHO = exports.formatWHO = function formatWHO(data) {
  var arr = [];

  for (var i = 0; i < data.fact.length; i++) {
    var country = data.fact[i].Dim.find(function (el) {
      return el.category === 'COUNTRY';
    });
    ...
    if (country) {
      ...
      newObj.title = data.dimension.find(function (el) {
        return el.display === 'Indicator';
      }).code[0].display;
      arr.push(newObj);
    }
  }
...
var formatSingleIndicator = exports.formatSingleIndicator = function formatSingleIndicator(data) {
  var facts = data.dimension.find(function (el) {
    return el.label === 'CHILDCAUSE';
  }).code;
  facts = facts.map(function (fact) {
    return { label: fact.label, display: fact.display.split(' <br> ').join(' '), value: 0 };
  });

  data.fact.forEach(function (el) {
    var label = el.Dim.find(function (cat) {
      return cat.category === 'CHILDCAUSE';
    }).code;
    facts.find(function (el2) {
      return el2.label === label;
    }).value += el.value.numeric;
  });

  facts.forEach(function (fact) {
    return fact.value = parseValue(fact.value);
  });
...
var formatMultipleIndicators = exports.formatMultipleIndicators = function formatMultipleIndicators(data) {
  var facts = [];

  data.forEach(function (f) {
    var indicator = f[0].dimension.find(function (el) {
      return el.display === 'Indicator';
    });
    if (indicator) {
      facts.push({
        label: indicator.code[0].label,
        display: indicator.code[0].display.split(' <br> ').join(' '),
  ...
  return facts.length > 0 ? facts.sort(function (a, b) {
    return sortFacts(a, b);
  }) : [{ label: 'null', display: 'No Data', value: 0 }];
};

var sortFacts = function sortFacts(a, b) {
...
var parseValue = function parseValue(value) {
  ...
  if (val.length === 2 && val[1].length > 2) {
    var parsedVal = val[0] + val[1].slice(0, 2);

    return parseInt(parsedVal) / 100.00;
  }

  return parseInt(val.join('.')) / 100.00;
};

var createGeoJSON = exports.createGeoJSON = function createGeoJSON(data) {
...
exports.whoList = undefined;

var _globe = __webpack_require__(2);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var dims = [doc.clientWidth - 10, doc.clientHeight - 65];
  var globe = new _globe2.default(dims);
  globe.setup();
});
...
var _world_map = __webpack_require__(3);

var _draw_util = __webpack_require__(4);

var DrawUtil = _interopRequireWildcard(_draw_util);

var _util = __webpack_require__(0);

var Util = _interopRequireWildcard(_util);

var _donut = __webpack_require__(5);
...
var Globe = function () {
  ...
  _createClass(Globe, [{
    ...
    value: function getItems() {
      ...
      this.startLoading();
      Util.fetchWHO(this.whoList[this.currentListItem]).then(function (data) {
        _this.items = Util.formatWHO(data);
        _this.worldMap = Util.bindMap(_world_map.worldMap, _this.items);
        _this.title = _this.worldMap.features.find(function (el) {
          return el.fact;
        }).fact.title;
        setTimeout(function () {
          _this.startGlobe();
          _this.setMouseActions();
        }, 1000);
      });
    }
  }, {
    ...
    key: 'setMouseActions',
    value: function setMouseActions() {
      this.canvas.call(d3.drag().on('start', this.dragStart.bind(this)).on('drag', this.dragging.bind(this)).on('end', this.dragEnd.bind(this))).on('mousemove', this.handleMouseMove).on('click', this.handleClick);
    }
  }, {
    key: 'checkDims',
    value: function checkDims() {
      if (!this.renderDims) {
    ...
    value: function handleClick() {
      ...
      if (this.findMenuItem(d3.event)) {
        var clickIndex = this.menuItems.indexOf(this.findMenuItem(d3.event));
        ...
        var country = this.findClickItem(d3.event);
        this.timer.stop();
        this.startLoading();

        (_$ = $).when.apply(_$, _toConsumableArray(Util.getCountry(this.whoList[this.currentListItem], country.id))).done(function () {
          for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
            args[_key] = arguments[_key];
          }

          var facts = args.every(function (el) {
            return Array.isArray(el);
          }) ? Util.formatMultipleIndicators(args) : Util.formatSingleIndicator(args[0]);
          _this2.setupMini();
          _this2.donut = new _donut2.default([_this2.dims[0], _this2.dims[1] - _this2.renderDims[1]], facts, country);
          _this2.donut.drawDonut();
        });
      }
    }
  }, {
    key: 'setMenuHighlight',
    value: function setMenuHighlight(item) {
      if (item) {
        this.menuHighlight = item;
        d3.select('canvas').classed('globe', false).classed('country', true);
      } else {
        this.menuHighlight = '';
        this.highlight = '';
        d3.select('canvas').classed('country', false).classed('globe', true);
      }
    }
  }, {
    key: 'setCountryHighlight',
    value: function setCountryHighlight(country) {
    ...
    value: function findMenuItem(event) {
      var x = event.layerX,
          y = event.layerY;
      var item = this.menuItems.find(function (el) {
        return x >= el.nw[0] && x <= el.se[0] && y >= el.nw[1] && y <= el.se[1];
      });
    ...
    key: 'findHoverItem',
    value: function findHoverItem(event) {
      var pos = this.globe.invert([event.layerX, event.layerY]);
      var country = this.worldMap.features.find(function (feature) {
        return d3.geoContains(feature, pos);
      });
      var menuItem = this.findMenuItem(event);
    ...
    key: 'findClickItem',
    value: function findClickItem(event) {
      if (this.highlight.features) {
        var country = this.highlight.features[0];
    ...
    key: 'setupMini',
    value: function setupMini() {
      this.renderDims = this.dims.map(function (dim) {
        return dim / 4;
      });
      d3.select('canvas').attr('width', this.dims[0] / 4).attr('height', this.dims[1] / 4);
      this.setGlobe();
      this.startGlobe();
    ...
    value: function scale() {
      ...
      this.globe.scale(0.8 * Math.min(width, height) / 2).translate([width * 2 / 5, height / 2]);
    }
  }, {
    key: 'dragStart',
    ...
    value: function dragging() {
      var xyz = [this.coords.x, this.coords.y, this.coords.z];
    ...
    key: 'setRotation',
    value: function setRotation(timeElapsed) {
      clearInterval(this.interval);
      var currentTime = d3.now();
      var timeDifference = currentTime - this.prevTime;

      if (timeDifference < timeElapsed) {
        var xyz = this.globe.rotate();
    ...
    value: function startLoading() {
      ...
      this.interval = setInterval(function () {
        _this3.renderLoading(int);
        int += 1;
      }, 300);
    }
  }, {
    key: 'findColor',
    value: function findColor(country) {
      var color = this.items.find(function (item) {
        return item.country = country.id;
      });
    ...
    key: 'renderLoading',
    value: function renderLoading(time) {
...
var drawObj = exports.drawObj = function drawObj(ctx, path, obj, color) {
  ctx.beginPath();
  path(obj);
  ctx.fillStyle = color;
  ctx.fill();
...
var drawLine = exports.drawLine = function drawLine(ctx, path, line, color) {
  ctx.beginPath();
  path(line);
...
var drawMap = exports.drawMap = function drawMap(ctx, path, worldMap, colorGrad) {
  worldMap.features.forEach(function (feature) {
    var val = void 0;
...
var getFittedText = function getFittedText(ctx, text, maxWidth) {
  text = text.split(' ');
  var lines = [];
  var line = '';
...
var drawTitle = exports.drawTitle = function drawTitle(ctx, text, dims) {
  var maxWidth = dims[0] / 4 - 100;
  var fittedText = getFittedText(ctx, text, maxWidth);
  var x = dims[0] * 3 / 4;
  var y = dims[1] / 3;

  fittedText.forEach(function (line) {
  ...
  [['No Data', '#0000ff'], ['Low', '#3366ff'], ['High', '#990000']].forEach(function (text) {
...
var drawMenu = exports.drawMenu = function drawMenu(ctx, dims, currentIndicator) {
  var x = dims[0] * 3 / 4;
  var y = dims[1] / 3 - 100;
  var colors = ['#0000ff', '#3366ff', '#990000'];
  ...
  menuItems = menuItems.map(function (item, idx) {
...
var drawHighlight = exports.drawHighlight = function drawHighlight(ctx, coords) {
  ctx.strokeStyle = 'yellow';
  ctx.lineWidth = 3;
  ctx.strokeRect(coords.nw[0] - 3, coords.nw[1] - 3, 16, 16);
};

var drawLoading = exports.drawLoading = function drawLoading(ctx, dims, time) {
  ctx.clearRect(0, 0, dims[0], dims[1]);
  var text = 'Loading from World Health Organization ...';
...
var Donut = function () {
  function Donut(dims, data, country) {
  ...
  _createClass(Donut, [{
    ...
    value: function setSVG() {
      return d3.select("#svg").append("svg").attr("width", this.width).attr("height", this.height).append("g").attr("transform", "translate(" + this.width / 3 + "," + this.height / 2 + ")");
    }
  }, {
    key: "getPie",
    ...
    key: "getLables",
    value: function getLables() {
      var _this = this;

      setTimeout(function () {
        _this.drawLabelBoxes();
        _this.drawLabels();
        _this.drawTitle();
      }, 500);
    }
    ...
    value: function drawPath() {
      ...
      var path = d3.select('g').selectAll('donut').data(this.getPie()(this.data)).enter().append('path').attr('d', this.getArc()).attr('fill', 'aliceblue').attr('class', 'donut').attr('id', function (d, i) {
        return "pie" + i;
      }).on('mouseover', function (d, i) {
        d3.select(this).style('opacity', 0.5);
        d3.selectAll("#label" + i).attr('font-size', '1.2em');
      }).on('mouseout', function (d, i) {
        d3.select(this).style('opacity', 1);
        d3.selectAll("#label" + i).attr('font-size', '0.8em');
    ...
    value: function drawTitle() {
      var x = this.width * 3 / 5;
      var y = 20;

      d3.select('svg').append('text').attr('x', x).attr('y', y).attr('font-size', '1.1em').text(this.country.properties.name + '  2015 Totals');
    }
  }, {
    key: "drawLabelBoxes",
    value: function drawLabelBoxes() {
      var _this3 = this;
      ...
      d3.select('svg').selectAll('rect').data(this.data).enter().append('rect').attr('x', x).attr('y', function () {
        y += 20;
        return y;
      }).attr('width', '0.8em').attr('height', '0.8em').attr('fill', 'aliceblue').transition().delay(function () {
        delay += 50;
        return delay;
      }).attr('fill', function (d, i) {
    ...
    value: function drawLabels() {
      var x = this.width * 3 / 5 + 20;
      var y = 20;

      d3.select('svg').selectAll('text').data(this.data).enter().append('text').attr('id', function (d, i) {
        return "label" + i;
      }).on('mouseover', function (d, i) {
        d3.select(this).attr('font-size', '1.2em');
        d3.select("#pie" + i).style('opacity', 0.5);
      }).on('mouseout', function (d, i) {
        d3.select(this).attr('font-size', '0.8em');
        d3.select("#pie" + i).style('opacity', 1);
      }).attr('x', x).attr('y', function () {
        y += 20;
        return y;
      }).attr('font-size', '0.8em').text(function (d) {
